...
var IMG_AJAXPATH = "/shop/admin/uploadimg";//异步传输服务端位置
...
$("#div_imgfile").click(function () {
    ...
    if ($(".imgfile").length <= $(".lookimg").length) {//个数不足则新创建对象
        ...
        _CRE_FILE.setAttribute("accept", ".png,.PNG,.JPG,.jpg,.jpeg");
...
$(".imgfile").live("change", function () {
    if ($(this).val().length > 0) {//判断是否有选中图片
        ...
        if (FORMAT !== "png" && FORMAT !== "jpg" && FORMAT !== "peg" && FORMAT !== "PNG" && FORMAT !== "JPG") {
            alert("文件格式不正确！！！");
            return;
        }
        ...
        if (file.size > (IMG_LENGTH * 3 * 1024 * 1024)) {
...
$("#btn_ImgUpStart").click(function () {
    ...
    for (var i = 0; i < $(".lookimg").length; i++) {
        ...
        $.ajax({
            type: "POST",
            url: IMG_AJAXPATH,
            data: { 'imgBase': IMG_BASE, 'imgFormat': IMG_FOMATE, 'lookIndex': NOWLOOK.index ,pnumber: pnumber},//图片base64编码，图片格式（当前仅支持jpg,png,jpeg三种），图片对象索引
            dataType: "json",
